...
public class WorkMadeDaoImpl extends BaseDaoImpl {
	...
	public boolean inSert(WorkMade workmade){
		...
		try {
			pstmt = conn.prepareStatement("insert into makework(work_name,work_content,start_time,teacher_name,course_name,work_number)values(?,?,?,?,?,?)");
			...
			pstmt.setInt(6, workmade.getWork_number());
			pstmt.executeUpdate();
			return true;
	...
	public List<WorkMade> QueryWorkT(String teacher_name) throws SQLException{
		...
		while(rs.next()){
			...
			workMade.setWork_number(rs.getInt(4));
